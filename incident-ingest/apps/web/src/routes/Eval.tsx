            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
              <div>
                ...
                <div style={{ fontSize: "0.85rem", color: "var(--text-muted)", display: "flex", gap: 16 }}>
                  <span>Status: <strong style={{ color: data.status === "passed" ? "var(--success)" : data.status === "failed" ? "var(--danger)" : "var(--warning)" }}>{data.status.toUpperCase()}</strong></span>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  {Object.entries(data.metrics).map(([key, value]) => {
                    ...
                    if (key.toLowerCase().includes("drop") || key.toLowerCase().includes("misscount") || key.toLowerCase().includes("rate")) {
                       if (threshold !== undefined && value != null) {
                         isPassing = value <= threshold; // reverse logic for drops/misses
                       }
                    }

                    return (
                      <div key={key} style={{ display: "flex", justifyContent: "space-between", paddingBottom: 8, borderBottom: "1px solid var(--border)" }}>
                        <span style={{ fontSize: "0.9rem" }}>{key}</span>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          {threshold !== undefined && (
                            <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>tgt: {key.includes("Count") ? threshold : formatPercent(threshold)}</span>
                          )}
              ...
              <div>
                ...
                <div style={{ fontSize: "0.85rem", lineHeight: 1.6, color: "var(--text-secondary)" }}>
                  <div><strong>Models:</strong> {data.models?.map(m => m.model).join(", ")}</div>
                ...
                {data.failures && data.failures.length > 0 && (
                  <div style={{ marginTop: 24 }}>
                    <h3 style={{ fontSize: "0.9rem", color: "var(--danger)", marginBottom: 12, textTransform: "uppercase" }}>Failures ({data.failures.length})</h3>
                    <div style={{ maxHeight: 200, overflowY: "auto", fontSize: "0.85rem", color: "var(--text-secondary)", padding: 12, background: "rgba(0,0,0,0.2)", borderRadius: 8 }}>
                      {data.failures.map((f, i) => (
                        <div key={i} style={{ marginBottom: 8 }}>
                          <strong>{f.queryId || "Unknown"}:</strong> {f.reason || JSON.stringify(f)}
                        </div>
                      ))}
                    </div>
                  </div>
